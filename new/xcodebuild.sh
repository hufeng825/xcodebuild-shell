...
scheme="" 						#打包的scheme
configuration="" 				#编译的方式,Release,Debug,Adhoc
app_display_name="" 			#应用的名字
app_bundle_identifier=""		#应用的Bundle ID
app_version="" 					#应用的version号
app_build="" 					#应用的build号
upload_app=true 				#是否上传App Store
#################################
...
project_path=$(cd `dirname $0`; pwd)o
xcodebuild_path="${project_path}/xcodebuild"
build_path="${xcodebuild_path}/${current_date}/build"
archive_path="${xcodebuild_path}/${current_date}/archive"
log_path="${xcodebuild_path}/${current_date}/log"
exportOptions_plist=${xcodebuild_path}/${current_date}/ExportOptions.plist
for file_name in `ls`;
do
...
function xcodebuild_project()
{
	...
	xcodebuild -project "${project_name}.xcodeproj" -scheme "${scheme}" -configuration "${configuration}" CONFIGURATION_BUILD_DIR="${build_path}/${build_direct_name}" >>"${log_path}"

	xcodebuild archive -project "${project_name}.xcodeproj" -scheme "${scheme}" -configuration "${configuration}" -archivePath "${build_path}/${scheme}.xcarchive" >>"${log_path}"

	xcodebuild_exportOptions_plist

	xcodebuild -exportArchive -archivePath "${build_path}/${scheme}.xcarchive" -exportPath "${archive_path}" -exportOptionsPlist "${exportOptions_plist}" >>"${log_path}"
}
 #打包workspace项目
function xcodebuild_workspace()
{
	
	xcodebuild clean -workspace "${project_name}.xcworkspace" -scheme "${scheme}" -configuration "${configuration}" >>"${log_path}"

	xcodebuild -workspace "${project_name}.xcworkspace" -scheme "${scheme}" -configuration "${configuration}" CONFIGURATION_BUILD_DIR="${build_path}/${build_direct_name}" >>"${log_path}"

	xcodebuild archive -workspace "${project_name}.xcworkspace" -scheme "${scheme}" -configuration "${configuration}" -archivePath "${build_path}/${scheme}.xcarchive" >>"${log_path}"
...
function upload_ipa_to_app_store
{
	...
	"${altool_path}" --validate-app -f ${archive_path}/${scheme}.ipa -u ${app_store_account} -p ${app_store_password} -t ios >>"${log_path}"
	
	echo -e	"\033[36m=== uploading... \033[0m"
	"${altool_path}" --upload-app -f ${archive_path}/${scheme}.ipa -u ${app_store_account} -p ${app_store_password} -t ios >>"${log_path}"
